...
public:
	...
	{
		m_file_servier=new FileServerLoop(port,ipv4_addr,ipv6_addr,MAX_THREADS_COUNT);
	}
	...
	void run()
	{
		...
		ALERT_RET(pthread_create(&t1,NULL,connection_loop,m_file_servier)==0,"Create Connect Thread Fail");
...
out:
	...
	p[p_size-1]=0;
	du[du_size-1]=0;
	delStringLastEnter(u);
	delStringLastEnter(p);
	delStringLastEnter(du);
	if(file!=NULL)
	{
		fclose(file);
