#ifndef __BOOK_MIND_SHARE__CMD_RESOLVER__
#define __BOOK_MIND_SHARE__CMD_RESOLVER__
...
template<class T>
class SafeQueueUnlimit:public SafeQueue<T>{
private:
	std::queue<T> m_queue;
	pthread_mutex_t m_mutex;
...
class CmdResolver:public CmdInfoListener{
private:
	SafeQueue<CmdData> *m_queue;
public:
	CmdResolver();
	CmdResolver(SafeQueue<CmdData> *queue);
	void setQueue(SafeQueue<CmdData> *queue);
	bool sqlInjectDetect(const char *src,const char *desc);
	void onCmdIn(CmdInfo *);
	void onConnectIn(SockData *);
	void onClose(SockData *);
};

class SystemResourceMonitor:public ResourceMonitor{
private:
	SafeQueue<CmdData> *m_queue;
public:
